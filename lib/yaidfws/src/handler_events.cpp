...
#pragma region HandlerEventSource
    ...
    AsyncWebHandlerEventSource ::AsyncWebHandlerEventSource(std::string url)
        : HandlerStaticUrl(url)
    {
        init();
    }
    
    AsyncWebHandlerEventSource ::AsyncWebHandlerEventSource(IHandlerContainer &handlerContainer, std::string url)
